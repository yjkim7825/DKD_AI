...
run_contrast <- function(alt, ref, tag) {
  ...
  ggsave(file.path(RES_DIR, paste0("vol_", tag, ".pdf")), p, width = 5.5, height = 4.5)
  cat(sprintf("[%s] DEG %d개 (up %d, down %d)\n",
              tag, nrow(sig), sum(sig$logFC > 0), sum(sig$logFC < 0)))
}

cat("그룹 구성: "); print(table(grp))
run_contrast(GROUP_LATE,  GROUP_EARLY,   "Late_vs_Early")     # Table S3
run_contrast(GROUP_LATE,  GROUP_CONTROL, "Late_vs_Control")   # Table S4
run_contrast(GROUP_EARLY, GROUP_CONTROL, "Early_vs_Control")  # Table S5

cat("\n[검증 팁] Late_vs_Early DEG 수를 논문값(대략 up 1,557 / down 1,276)과 비교해보세요.\n")
cat("         figshare의 'Supplementary Table 3'(diff late vs early)와 유전자 목록도 대조 가능.\n")

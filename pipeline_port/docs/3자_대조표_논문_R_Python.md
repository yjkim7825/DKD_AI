...
| **FN1** | valid | 0.911 | 0.915 | 0.915 |
| **ALDH2** | train | 0.912 | 0.940 | 0.940 |
| **ALDH2** | valid | 0.815 | 0.784 | 0.784 |

- **Python = R 소수점까지 완전 일치** (동일 매트릭스·동일 판별식, ALDH2는 보호유전자라 방향 자동보정).
- FN1은 논문과도 거의 정확, ALDH2는 근사(train↑/valid↓).
...
| GLM | valid | **0.942** | 0.826 | **0.926** |
| RF | train | 1.000 | 1.000 | 1.000 |
| RF | valid | 0.815 | 0.914 | 0.913 |
| SVM | train | 0.977 | 0.973 | 0.978 |
| SVM | valid | 0.807 | 0.785 | **0.405** ⚠️ |
| XGBoost | train | 0.999 | 0.997 | 0.997 |
| XGBoost | valid | 0.844 | 0.873 | 0.873 |

- 논문의 최종 진단모델은 이 결합모델(특히 GLM 0.942)임.
- **Python GLM(0.926)이 R(0.826)보다 논문(0.942)에 근접.**
- **SVM valid(0.405)만 이탈** — sklearn `SVC(rbf, gamma=scale)` ≠ R `e1071`(커널·스케일 규약 차이). 포팅 한계로 기록.
